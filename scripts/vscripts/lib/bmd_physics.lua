...
function Physics:start()
  ...
  GameRules:GetGameModeEntity():SetThink("Think", self, "physics", PHYSICS_THINK)
end
...
function Physics:AngleGrid( angoffsets )
  local anggrid = {}
  ...
  if blocked then
    for k=1,#vecs do
      ...
      local blo = not GridNav:IsTraversable(pos) or GridNav:IsBlocked(pos)
    ...
    if seg > 1 then
      --print ('OVERSEG x=' .. GridNav:GridPosToWorldCenterX(i) .. ' y=' .. GridNav:GridPosToWorldCenterY(j))
      anggrid[i+offsetX][j+offsetY] = -1
    elseif count > 5 then
      --print ('PROTRUDE x=' .. GridNav:GridPosToWorldCenterX(i) .. ' y=' .. GridNav:GridPosToWorldCenterY(j))
      anggrid[i+offsetX][j+offsetY] = -1
    elseif count == 0 then
      anggrid[i+offsetX][j+offsetY] = -1
    else
