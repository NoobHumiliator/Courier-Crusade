...
function item_black_king_bar_datadriven_on_spell_start(keys)
	...
	local final_model_scale = 2.6  --This will be something like 1.3.
	local model_scale_increase_per_interval = 100 / (final_model_scale - 1.6)

	--Scale the model up over time.
	for i=1,100 do
		Timers:CreateTimer(i/75, 
		function()
			keys.caster:SetModelScale(1.6 + i/model_scale_increase_per_interval)
		end)
	end
